...
def get_media_duration(audio_file: Path) -> float:
    ...
    error_console.print(f'[error]Unable not parse duration for file: {audio_file}, ffmpeg output\n: {output}')
    raise SystemExit(1)
...
def convert_to_wav(audio_file: Path) -> Path:
    ...
    try:
        ...
        error_console.print(f'[error]Unable to convert {audio_file}, ffmpeg error:\n {e.stdout.decode()}')
